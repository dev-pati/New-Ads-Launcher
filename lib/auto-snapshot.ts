...
async function snapshotCampaigns(orgId: string, actId: string, token: string, days: number) {
  ...
  const fields = [
    "campaign_id","campaign_name","spend","impressions","inline_link_clicks","reach","frequency",
    "actions","action_values","purchase_roas","cpm","cpc","inline_link_click_ctr",
    "outbound_clicks","date_start",
  ].join(",")
  ...
  const rows = insights.map((ins: any) => {
    ...
    return {
      ...
      inline_link_clicks:  parseInt(ins.inline_link_clicks ?? "0") || 0,
      clicks:              parseInt(ins.inline_link_clicks ?? "0") || 0,
      reach:               parseInt(ins.reach ?? "0") || 0,
      frequency:           parseFloat(ins.frequency ?? "0") || null,
      outbound_clicks:     parseInt((ins.outbound_clicks ?? [])[0]?.value ?? "0") || 0,
      ...
      ctr: parseFloat(ins.inline_link_click_ctr ?? "0") || 0,
...
async function snapshotAds(orgId: string, actId: string, token: string, days: number) {
  ...
  const fields = [
    ...
    "spend","impressions","inline_link_clicks","inline_link_click_ctr","reach","frequency",
    "actions","action_values","purchase_roas","cpm","cpc","outbound_clicks",
  ...
  const rows = insights.map((ins: any) => {
    ...
    return {
      ...
      frequency:              parseFloat(ins.frequency ?? "0") || null,
      outbound_clicks:        parseInt((ins.outbound_clicks ?? [])[0]?.value ?? "0") || 0,
      inline_link_click_ctr:  parseFloat(ins.inline_link_click_ctr ?? "0") || 0,
      purchases, purchase_value: purchaseVal,
      leads:       sumAction(ins.actions, LEAD_TYPES),
      add_to_carts: sumAction(ins.actions, ["add_to_cart"]),
      roas: parseFloat((ins.purchase_roas ?? [])[0]?.value ?? "0") || null,
      ...
      video_p25_watched:    parseInt((ins.video_p25_watched_actions ?? [])[0]?.value ?? "0") || 0,
      video_p50_watched:    parseInt((ins.video_p50_watched_actions ?? [])[0]?.value ?? "0") || 0,
      video_p75_watched:    parseInt((ins.video_p75_watched_actions ?? [])[0]?.value ?? "0") || 0,
      video_p95_watched:    parseInt((ins.video_p95_watched_actions ?? [])[0]?.value ?? "0") || 0,
      video_p100_watched:   parseInt((ins.video_p100_watched_actions ?? [])[0]?.value ?? "0") || 0,
      video_30s_watched:    parseInt((ins.video_30_sec_watched_actions ?? [])[0]?.value ?? "0") || 0,
      raw_insights: ins, snapped_at: now,
    }
  })
  const db = createAdminClient()
...
async function snapshotAdsets(orgId: string, actId: string, token: string, days: number) {
  ...
  const fields = [
    ...
    "spend","impressions","inline_link_clicks","reach","frequency",
    "actions","action_values","purchase_roas","cpm","cpc","inline_link_click_ctr","outbound_clicks","date_start",
  ].join(",")
  const data = await metaFetch(
