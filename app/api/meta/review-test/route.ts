...
async function runValidatedCampaignCreate(adAccountId: string, accessToken: string): Promise<ReviewCall> {
  ...
  try {
    ...
    if (!res.ok || data?.error) {
      const fb = data?.error
      const detail = [fb?.message, fb?.error_user_title, fb?.error_user_msg, fb?.type, fb?.code ? `code=${fb.code}` : ""]
        .filter(Boolean)
        .join(" | ")
    ...
    return {
      ...
      ok: true,
      status: res.status,
      note: "validate_only: does not create a real campaign",
      data,
    }
  } catch (err: any) {
    return {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pagesCall = await runGet(
      "pages_show_list",
      "/me/accounts?fields=id,name,access_token&limit=5",
    ...
    if (page?.id) {
      calls.push(await runGet(
        "pages_read_engagement",
        `/${page.id}?fields=id,name,fan_count,posts.limit(1){id,created_time,message}`,
        page.access_token || connection.access_token,
        `page_id=${page.id}`
      ))
    } else {
      calls.push({
        ...
        method: "GET",
        endpoint: "/{page_id}?fields=id,name,fan_count,posts.limit(1){id,created_time,message}",
        ok: false,
        status: 404,
        error: "No Page returned by /me/accounts. Connect/select a user with Page access.",
      })
    }
    ...
    calls.push(await runGet(
      "ads_read",
      `/${actId}/campaigns?fields=id,name,status,effective_status,created_time&limit=10`,
      connection.access_token
    ))
    ...
    return NextResponse.json({
      ...
      summary: {
        ok: okCount,
        failed: calls.length - okCount,
        total: calls.length,
        adAccountId: actId,
        adsManagementMode: "validate_only_no_campaign_created",
      },
      calls,
    })
  } catch (err: any) {
    console.error("[meta/review-test] error:", err)
    return NextResponse.json({ error: err.message || "Failed to run Meta review test" }, { status: 500 })
  }
}
