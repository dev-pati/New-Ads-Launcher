...
export interface AdsetTextConfig {
  adsetName: string
  headlines: string[]
  primaryTexts: string[]
  description: string
...
function defaultConfig(adsetName: string): AdsetTextConfig {
  return { adsetName, headlines: [""], primaryTexts: [""], description: "", cta: "LEARN_MORE", websiteUrl: "", displayUrl: "" }
}
...
interface Props {
  ...
  onClose: () => void
  adsetNames: string[]
  initial: AdsetTextConfig[]
  onApply: (configs: AdsetTextConfig[]) => void
}

export function AdSetTextDialog({ open, onClose, adsetNames, initial, onApply }: Props) {
  const [configs, setConfigs] = useState<AdsetTextConfig[]>([])

  useEffect(() => {
    if (!open) return
    const next = adsetNames.map((name, i) => {
      const existing = initial.find(c => c.adsetName === name) || initial[i]
      return existing ? { ...existing, adsetName: name } : defaultConfig(name)
    })
    setConfigs(next)
  }, [open, adsetNames.join("|")])

  const update = (i: number, patch: Partial<AdsetTextConfig>) =>
    setConfigs(prev => prev.map((c, idx) => idx === i ? { ...c, ...patch } : c))

  const resetAll = () => setConfigs(adsetNames.map(defaultConfig))
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-[98vw] max-w-[98vw] h-[92vh] max-h-[92vh] p-0 overflow-hidden flex flex-col gap-0">
        <DialogHeader className="px-6 py-4 border-b shrink-0">
          <DialogTitle className="text-xl">Edit Ad Set Text</DialogTitle>
          <p className="text-sm text-muted-foreground">Configure unique text for each ad set</p>
        </DialogHeader>
