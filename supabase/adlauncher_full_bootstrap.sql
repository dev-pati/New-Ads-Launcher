-- ============================================================
-- ADLAUNCHER FULL DATABASE BOOTSTRAP
-- Target schema: ads_launcher.
-- Supabase Auth dependency is replaced by ads_launcher.accounts.
--
-- This file is the one-shot SQL bootstrap for a fresh database.
-- It includes the current schema snapshot plus post-snapshot tables:
--   - launch_drafts
--   - google_connections
--   - meta_api_cache
--
-- WARNING: This script drops and recreates objects in ads_launcher.
-- Do not run it on production unless you intentionally want to rebuild
-- the schema and have a verified backup.
-- ============================================================
...
DROP TABLE IF EXISTS ads_launcher.google_connections CASCADE;
...
DROP TABLE IF EXISTS ads_launcher.launch_drafts CASCADE;
...
CREATE INDEX idx_meta_api_cache_org_key ON meta_api_cache(org_id, cache_key);
CREATE INDEX idx_meta_api_cache_expires_at ON meta_api_cache(expires_at);
...
CREATE INDEX launch_drafts_user_id_idx ON launch_drafts(user_id, created_at DESC);
...
CREATE TRIGGER trg_launch_drafts_updated_at        BEFORE UPDATE ON launch_drafts        FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trg_google_connections_updated_at   BEFORE UPDATE ON google_connections   FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trg_meta_api_cache_updated_at       BEFORE UPDATE ON meta_api_cache       FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- PROFILES
CREATE POLICY "Users can view own profile"         ON profiles FOR SELECT USING (id = current_account_id());
CREATE POLICY "Users can update own profile"       ON profiles FOR UPDATE USING (id = current_account_id());
CREATE POLICY "Users can view org member profiles" ON profiles FOR SELECT
  USING (id IN (SELECT user_id FROM org_members WHERE org_id IN (SELECT org_id FROM org_members WHERE user_id = current_account_id())));
...
CREATE POLICY "Users can create orgs"  ON organizations FOR INSERT WITH CHECK (created_by = current_account_id());
...
CREATE POLICY "Creator can add self"       ON org_members FOR INSERT WITH CHECK (user_id = current_account_id());
...
-- USER SETTINGS
CREATE POLICY "Users can view own settings"   ON user_settings FOR SELECT USING (user_id = current_account_id());
CREATE POLICY "Users can insert own settings" ON user_settings FOR INSERT WITH CHECK (user_id = current_account_id());
CREATE POLICY "Users can update own settings" ON user_settings FOR UPDATE USING (user_id = current_account_id());
...
-- LAUNCH DRAFTS
CREATE POLICY "Org members can manage launch drafts" ON launch_drafts FOR ALL USING (is_org_member(org_id));
...
-- GOOGLE CONNECTIONS
CREATE POLICY "Org members can manage google connections" ON google_connections FOR ALL USING (is_org_member(org_id));

-- META API CACHE
CREATE POLICY "Org members can read meta api cache" ON meta_api_cache FOR SELECT USING (is_org_member(org_id));

-- NOTIFICATIONS
CREATE POLICY "Users can read own notifications"   ON notifications FOR SELECT USING (current_account_id() = user_id);
CREATE POLICY "Users can update own notifications" ON notifications FOR UPDATE USING (current_account_id() = user_id);
