...
CREATE TYPE org_role AS ENUM ('admin', 'editor', 'launcher', 'uploader', 'analyst', 'commenter');
...
-- ============================================================
-- POST-SCHEMA TABLES (migrations 20260515 - 20260603)
-- ============================================================

-- launch_drafts (20260515)
...
CREATE INDEX IF NOT EXISTS launch_drafts_org_id_idx ON launch_drafts (org_id, created_at DESC);

-- launch_batches.page_name (20260520)
ALTER TABLE launch_batches ADD COLUMN IF NOT EXISTS page_name text;

-- google_connections (20260520)
...
CREATE INDEX IF NOT EXISTS idx_meta_api_cache_org_key ON meta_api_cache(org_id, cache_key);
CREATE INDEX IF NOT EXISTS idx_meta_api_cache_expires_at ON meta_api_cache(expires_at);
...
-- ad_accounts new columns (20260527)
...
-- ad_account_metrics_snapshots (20260527)
...
CREATE INDEX IF NOT EXISTS idx_ad_account_metrics_snapshots_org ON ad_account_metrics_snapshots(org_id, synced_at DESC);
CREATE INDEX IF NOT EXISTS idx_ad_account_metrics_snapshots_account ON ad_account_metrics_snapshots(org_id, fb_ad_account_id, synced_at DESC);
ALTER TABLE ad_account_metrics_snapshots ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Org members can view ad account metric snapshots"   ON ad_account_metrics_snapshots FOR SELECT USING (is_org_member(org_id));
CREATE POLICY "Org members can insert ad account metric snapshots" ON ad_account_metrics_snapshots FOR INSERT WITH CHECK (is_org_member(org_id));
CREATE POLICY "Org admins can delete ad account metric snapshots"  ON ad_account_metrics_snapshots FOR DELETE USING (is_org_admin(org_id));
...
-- campaign_insights_snapshots (20260603)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_campaign_insights_unique ON campaign_insights_snapshots(org_id, fb_campaign_id, date);
CREATE INDEX IF NOT EXISTS idx_campaign_insights_org_date ON campaign_insights_snapshots(org_id, date DESC);
CREATE INDEX IF NOT EXISTS idx_campaign_insights_account ON campaign_insights_snapshots(org_id, fb_ad_account_id, date DESC);
ALTER TABLE campaign_insights_snapshots ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Org members can view campaign snapshots"   ON campaign_insights_snapshots FOR SELECT USING (is_org_member(org_id));
CREATE POLICY "Org members can insert campaign snapshots" ON campaign_insights_snapshots FOR INSERT WITH CHECK (is_org_member(org_id));
CREATE POLICY "Org members can update campaign snapshots" ON campaign_insights_snapshots FOR UPDATE USING (is_org_member(org_id));