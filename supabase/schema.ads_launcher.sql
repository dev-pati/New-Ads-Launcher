-- ============================================================
-- ADLAUNCHER SELF-HOSTED SCHEMA (CANONICAL)
-- Target schema: ads_launcher.
-- Custom auth: ads_launcher.accounts + current_account_id().
-- schema.sql is legacy Supabase-Auth lineage — do not apply to prod.
-- Live deltas: supabase/migrations/* (apply in date order).
-- ============================================================
...
CREATE POLICY "Can edit scheduled activations insert" ON scheduled_activations FOR INSERT WITH CHECK (can_edit_ads(org_id));
CREATE POLICY "Can edit scheduled activations update" ON scheduled_activations FOR UPDATE USING (can_edit_ads(org_id));
CREATE POLICY "Can edit scheduled activations delete" ON scheduled_activations FOR DELETE USING (can_edit_ads(org_id));
...
CREATE POLICY "Can upload asset boards insert" ON asset_boards FOR INSERT WITH CHECK (can_upload_media(org_id));
CREATE POLICY "Can upload asset boards update" ON asset_boards FOR UPDATE USING (can_upload_media(org_id));
